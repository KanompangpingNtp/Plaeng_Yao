...
<h2 class="text-center">ข้อมูลสภาพทั่วไป</h2><br>

<div class="card mt-3">
    <div class="card-body">
        @forelse ($basicInfoDetail as $detail)
        <form action="{{ route('GeneralInformationDelete', $detail->id) }}" method="POST" onsubmit="return confirm('คุณต้องการลบข้อมูลนี้หรือไม่?');">
            @csrf
            @method('DELETE')
...
<form action="{{route('GeneralInformationCreate')}}" method="POST" enctype="multipart/form-data">
    @csrf
    <div>
        <input type="hidden" name="basic_info_type" value="{{ $basicInfoType->firstWhere('type_name', 'ข้อมูลสภาพทั่วไป')->id }}">
    </div>
    ...
    <button class="btn btn-primary" type="submit" disabled>สร้างข้อมูลแล้ว</button>
    @endif
</form>
