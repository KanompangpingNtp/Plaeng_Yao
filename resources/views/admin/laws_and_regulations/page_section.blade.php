...
<h3 class="text-center">สร้างหัวข้อย่อย <br> <span class="text-primary">{{$LawsRegsType->type_name}}</span></h3><br>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="margin-top: 5%;">
        <div class="modal-content">
            ...
            <form action="{{ route('LawsAndRegulationsSectionCreate',$LawsRegsType->id) }}" method="POST">
                @csrf
...
<table class="table table-bordered text-center" id="data_table">
    ...
    <tbody>
        @foreach ($LawsRegsSection as $index => $details)
        <tr>
            <td>{{ $index + 1 }}</td>
            <td>
                <a href="{{ route('LawsAndRegulationsShowDetails', $details->id) }}">
            ...
            <td>
                ...
                <form action="{{ route('LawsAndRegulationsSectionDelete', $details->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
...
@foreach ($LawsRegsSection as $details)
<div class="modal fade" id="editModal-{{ $details->id }}" tabindex="-1" aria-labelledby="editModalLabel-{{ $details->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="margin-top: 5%;">
        <div class="modal-content">
            ...
            <form action="{{ route('LawsAndRegulationsSectionUpdate', $details->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="mb-3">
