    ...
    <div class="container py-5 custom-gradient-shadow">
        <div class=" d-flex flex-column justify-content-center p-5">
            <div class="fs-1 fw-bold mb-4 text-center">
                {{ $OperationalPlanSection->type->type_name }} <br> <span style="font-size: 30px;">{{ $OperationalPlanSection->section_name }}</span>
            </div>

            <!-- ไฟล์แนบ: PDF -->
            @if ($OperationalPlanFile->count() > 0)
            @foreach ($OperationalPlanFile as $details)
            <div class="mb-3">
