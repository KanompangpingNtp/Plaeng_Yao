...
<h3 class="text-center">ข้อมูล <br> <span class="text-primary">{{$PublicMenusSection->section_name}}</span> </h3><br>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="margin-top: 5%;">
        <div class="modal-content">
            ...
            <form action="{{ route('MenuForPublicDetailCreate',$PublicMenusSection->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<table class="table table-bordered text-center" id="data_table">
    ...
    <tbody>
        @foreach ($PublicMenusFiles as $index => $details)
        <tr>
            <td>{{ $index + 1 }}</td>
            <td>
                <!-- แสดงชื่อไฟล์ -->
            ...
            <td>
                ...
                <form action="{{ route('MenuForPublicDetailDelete', $details->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
