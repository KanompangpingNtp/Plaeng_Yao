...
<h2 class="text-center">วิสัยทัศน์/พันธกิจ</h2><br>

<div class="card mt-3">
    <div class="card-body">
        @forelse ($basicInfoDetail as $detail)
        <form action="{{ route('VisionMissionDelete', $detail->id) }}" method="POST" onsubmit="return confirm('คุณต้องการลบข้อมูลนี้หรือไม่?');">
            @csrf
            @method('DELETE')
...
<form action="{{route('VisionMissionCreate')}}" method="POST" enctype="multipart/form-data">
    @csrf
    <div>
        <input type="hidden" name="basic_info_type" value="{{ $basicInfoType->firstWhere('type_name', 'วิสัยทัศน์/พันธกิจ')->id }}">
    </div>
