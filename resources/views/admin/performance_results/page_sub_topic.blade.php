...
<h3 class="text-center">สร้างหัวข้อย่อย <br> <span class="text-primary">{{$PerfResultsSection->section_name}}</span></h3><br>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                <form action="{{ route('PerfResultsSubTopicDelete', $details->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
            ...
            <form action="{{ route('PerfResultsSubTopicUpdate', $details->id) }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="topic_name-{{ $details->id }}" name="topic_name" value="{{ $details->topic_name }}">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
