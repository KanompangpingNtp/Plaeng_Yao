...
<h3 class="text-center">สร้างหัวข้อเมนูสำหรับประชาชน</h3><br>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="margin-top: 5%;">
        <div class="modal-content">
            ...
            <form action="{{ route('MenuForPublicTypeCreate') }}" method="POST">
                @csrf
...
<table class="table table-bordered text-center" id="data_table">
    ...
    <tbody>
        @foreach ($PublicMenusType as $index => $details)
        <tr>
            <td>{{ $index + 1 }}</td>
            <td>
                <a href="{{ route('MenuForPublicShowSection', $details->id) }}">
                    {{ $details->type_name }}
                </a>
            </td>
            <td>
                ...
                <form action="{{ route('MenuForPublicDelete', $details->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
...
@foreach ($PublicMenusType as $details)
<div class="modal fade" id="editModal-{{ $details->id }}" tabindex="-1" aria-labelledby="editModalLabel-{{ $details->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="margin-top: 5%;">
        <div class="modal-content">
            ...
            <form action="{{ route('MenuForPublicUpdate', $details->id) }}" method="POST">
                @csrf
                <div class="modal-body">
                    <div class="mb-3">
