...
<div class="bg py-5">
    <div class="container py-5 custom-gradient-shadow">
        <div class=" d-flex flex-column justify-content-center p-5">
            <div class="fs-1 fw-bold mb-4 text-center" style="color: #77b329;">สรุปผลการจัดซื้อจัดจ้าง <br><span class="fs-3">{{$revenue->title_name}}</div>

                <p class="text-muted">วันที่เผยแพร่: {{ \Carbon\Carbon::parse($revenue->date)->format('d-m-Y') }}</p>

                <!-- ไฟล์แนบ: PDF -->
                @if ($revenue->pdfs->count() > 0)
                <h5 class="text-secondary mt-4">ไฟล์เอกสาร</h5>
                @foreach ($revenue->pdfs as $pdf)
                <div class="mb-3">
