...
<h3 class="text-center">สร้างหัวข้อย่อย <br> <span class="text-primary">{{$PublicMenusType->type_name}}</span></h3><br>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="margin-top: 5%;">
        <div class="modal-content">
            ...
            <form action="{{ route('MenuForPublicSectionCreate',$PublicMenusType->id) }}" method="POST">
                @csrf
...
<table class="table table-bordered text-center" id="data_table">
    ...
    <tbody>
        @foreach ($PublicMenusSection as $index => $details)
        <tr>
            <td>{{ $index + 1 }}</td>
            <td>
                <a href="{{ route('MenuForPublicShowDetails', $details->id) }}">
            ...
            <td>
                ...
                <form action="{{ route('MenuForPublicSectionDelete', $details->id) }}" method="POST" style="display:inline;">
                    @csrf
                    @method('DELETE')
...
@foreach ($PublicMenusSection as $details)
<div class="modal fade" id="editModal-{{ $details->id }}" tabindex="-1" aria-labelledby="editModalLabel-{{ $details->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="margin-top: 5%;">
        <div class="modal-content">
            ...
            <form action="{{ route('MenuForPublicSectionUpdate', $details->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="mb-3">
